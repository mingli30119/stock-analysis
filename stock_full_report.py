...
try:
    ...
    print("[!] 未找到 technical_indicators.py，技术指标计算将被跳过", file=sys.stderr)
...
def collect(code: str, max_kline_years: int = 3) -> StockReportData:
    ...
    print("[3/13] 行情 K 线")
    df_kline = _safe_call(ak.stock_zh_a_daily, symbol=prefixed,
                          start_date=kline_start, end_date=today_s, adjust="qfq",
                          label=f"新浪-日K（{max_kline_years}年前复权）")

    # 保存原始K线数据
    data.blocks["kline_daily"] = _df_to_records(df_kline)

    # 计算技术指标
    if df_kline is not None and not df_kline.empty and calculate_all_indicators is not None:
        print("   [3.1/13] 计算技术指标（MACD/KDJ/RSI/BOLL/MA）")
        try:
            # 标准化列名（akshare 返回的列名可能不同）
            ...
            for old_col, new_col in column_mapping.items():
                if old_col in df_kline_copy.columns:
                    df_kline_copy.rename(columns={old_col: new_col}, inplace=True)

            # 确保必需列存在
            required_cols = ['date', 'open', 'high', 'low', 'close', 'volume']
            if all(col in df_kline_copy.columns for col in required_cols):
                # 计算所有技术指标
                df_with_indicators = calculate_all_indicators(df_kline_copy)

                # 格式化为 JSON（最近120条）
                data.blocks["technical_indicators"] = format_indicators_for_json(df_with_indicators, max_rows=120)

                # 获取最新技术信号
                data.blocks["technical_signals"] = get_latest_signals(df_with_indicators)

                print(f"   ✓ 技术指标计算完成：{len(data.blocks['technical_indicators'])} 条记录")
            else:
                print(f"   ✗ K线数据列不完整，跳过技术指标计算")
                data.blocks["technical_indicators"] = []
                data.blocks["technical_signals"] = {}
        except Exception as e:
            print(f"   ✗ 技术指标计算失败: {e}")
            data.blocks["technical_indicators"] = []
            data.blocks["technical_signals"] = {}
