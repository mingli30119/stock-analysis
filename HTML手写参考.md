...
16. .card#technical — 技术面分析（第9章，含信号总览5卡片+MACD/KDJ/RSI/BOLL四图表）
17. footer          — 页脚（固定模板，禁止修改结构）
```
...
<p>📅 分析基准日：{YYYY-MM-DD} · 数据截止：{日期} · 研究状态：{首次覆盖/持续跟踪}</p>
<p style="font-size:11px;color:var(--text-muted);">数据来源：akshare(雪球/新浪/同花顺/东方财富/巨潮资讯) · GLM5.1 · DeepSeek-V4-Pro · Claude Opus-4.7 · 2026-05-03 · @明立玩AI · 个股深度研究系统 v3.0</p>
...
```html
<nav class="top-nav">
<div class="logo"><div class="logo-icon">股</div><span class="stock-name">股票名</span><span class="stock-code">代码</span></div>
<div class="nav-links">
<!-- 13个固定链接 + 可选1个技术面链接 -->
</div>
<span class="nav-analysis-date">📅 YYYY-MM-DD</span>
<button class="theme-toggle" id="themeToggle">☀️ 浅色模式</button>
</nav>
```

### 导航标签（13个固定文本 + 1个可选，禁止修改文本）

```
行情 | 结论 | 画像 | K线 | 任务 | 宏观 | 产业链 | 质量 | 弹性 | 风险 | 估值 | 对标 | 跟踪 | 技术面
```

对应锚点：`#hero` `#conclusion-top` `#profile` `#kline-section` `#mission` `#macro` `#chain` `#quality` `#elasticity` `#risk` `#valuation` `#compare` `#tracking` [`#technical`]

**nav中包含分析基准日：** `<span class="nav-analysis-date">📅 YYYY-MM-DD</span>` 放在nav-links右侧、theme-toggle按钮左侧，12px字号，颜色跟随主题（CSS class: `nav-analysis-date`）。

## 组件速查表

| 组件 | HTML结构 | CSS class | 注意事项 |
|------|----------|-----------|---------|
| **Hero** | `.hero > .hero-price-block + .hero-meta + .hero-tags` | `hero`, `hero-price`, `hero-meta-item` | 价格56px/900weight。**hero-meta 5项固定顺序**：①分析基准日（`YYYY-MM-DD`，标签"分析基准日"，颜色跟随主题）②质量评级（含分数，标签"公司质地"）③中期目标价 ④盈亏比 ⑤风险等级。右上角径向光晕。**hero-tags ≤5个标签，每标签≤4个中文字**，用`.hero-tag`class |
...
| **新闻列表** | `.news-item` 每条一个flex行容器 | `news-item` | **禁止使用滚动容器**（`max-height`/`overflow-y`），固定高度展示。每条新闻为纯文本 `<span>`（暂不设链接）。每条右侧必须有**利好/利空/中性标签**（`<span>` + 内联style）：利好=红底红字（`background:rgba(245,86,86,0.15);color:var(--red-up)`），重大利好=红底红字+红色左边框（`border-left:3px solid var(--red-up)`），中性=金底金字（`background:rgba(212,168,83,0.15);color:var(--gold)`），利空=绿底绿字。标签文字≤4字。左边框色跟随标签语义。**增量更新时，整个新闻列表替换为当日最新8条新闻，重新判断利好利空属性** |
| **信息网格** | `dl.info-grid > dt + dd` | `info-grid` | 两列，dt灰色/dd白色 |
| **KPI行** | `.kpi-info-row > .kpi-info-item` | `kpi-info-row`, `kpi-info-item` | K线图下方4指标，分隔线 |
| **K线技术标注** | ECharts `markLine` + `markPoint` | — | **强制组件**。4条markLine（虚线金色）：前高压力/MA20或关键均线/加仓位/止损位，价格取自MD报告分析。2个markPoint（pin图标）：当前价位(金色)+关键历史价位(红色)。label用 `#fff` 字号11px。markLine label用 `#d4a853` 字号10px |
| **跟踪日志表** | `table.cons` 催化剂追踪 | `cons` | **Step 8 强制组件**。表头：`跟踪指标/催化剂 | 预期影响 | 兑现状态 | 备注`。每行 = 一个从MD报告中提取的关键催化剂/事件。状态标记：✅已兑现(红) / ⏳待兑现(金) / ❌落空(绿) / ⚪持续观察(灰)。首次分析选6-10个可验证事件。增量更新时逐行检查兑现状态（不插新列，更新状态列即可），有新催化剂则追加新行。表格下方必须有 `📋 事件记录` 区块（`border-left:4px solid var(--gold)`），倒序列出每次更新的发现。**增量更新时必须同步刷新近期动态（替换为当日最新8条新闻+重新判断利好利空）** |
...
| 3. Grep | — | `grep -oc 'href="#' _h_part2.html` | = 13（或14含技术面） |
...
| 3. Grep | — | `grep 'nav-analysis-date' _h_part2.html` | 存在（分析基准日在nav） |
| 3. Grep | — | `grep 'news-list' _h_part2.html` | 存在 |
| 3. Grep | — | `grep -c '利好\|利空\|中性' _h_part2.html` | ≥ 5（每条新闻有情绪标签） |
| 3. Grep | — | `grep 'overflow-y\|max-height:300' _h_part2.html` | = 0（禁止滚动容器） |
| 3. Grep | — | `grep -c 'style="[^"]*color:#fff[^"]*"\|style="[^"]*color:white[^"]*"' _h_part2.html` | = 0（禁止内联硬编码白色） |
...
| 3. Grep | — | `grep -c 'markLine' _h_part3.html` | ≥ 1（K线技术标注） |
| 3. Grep | — | `grep -c 'markPoint' _h_part3.html` | ≥ 1（K线关键点位） |
...
| 3. Grep | — | `grep '催化剂兑现追踪\|已兑现\|待兑现' _h_part6.html` | 存在（催化剂追踪表） |
| 3. Grep | — | `grep '📋 事件记录' _h_part6.html` | 存在（事件记录区块） |
...
| 步骤 | 操作 | 校验命令 | 通过条件 |
|------|------|---------|---------|
| 1. Read | `Read 中国长城 line=864-1155`（script段结构，含技术指标计算+6图表渲染） | — | — |
...
> **每条检查都是已知 Bug 的防御。全部 16 条通过才可输出最终文件。**
> **失败 → grep 定位问题 → Edit 修正 → 重跑该条，不推导原因。**

| # | 检查项 | 命令 | 通过条件 | Bug 防御 |
|---|--------|------|---------|---------|
| 1 | 导航锚点配对 | `grep -oc 'href="#' 个股研究-XXX.html` + `grep -o 'id="[^"]*"' \| grep -cE 'hero\|conclusion-top\|...'` | nav数 = 14（含技术面）, 关键id = 14 | 导航死链 |
...
| 10 | Footer 结构完整 | `grep -c '免责声明' 个股研究-XXX.html` + `grep -c '作者：明立' 个股研究-XXX.html` + `grep -c 'll-mingli1221' 个股研究-XXX.html` + `grep -c '分析基准日' 个股研究-XXX.html` | 全部 ≥ 1，分析基准日≥3（nav+hero+footer三处） | Footer 丢失 |
| 11 | 内联硬编码白色清零 | `grep -c 'style="[^"]*color:#fff[^"]*"\|style="[^"]*color:white[^"]*"\|style="[^"]*color:rgba(255' 个股研究-XXX.html` | = 0 | **Bug 5: 浅色模式白字不可见** |
| 12 | 新闻情绪标签齐全 | `grep -c '利好\|利空\|中性' 个股研究-XXX.html` | ≥ 5 | 新闻无情绪标注 |
| 13 | K线技术标注 | `grep -c 'markLine' 个股研究-XXX.html` + `grep -c 'markPoint' 个股研究-XXX.html` | ≥ 1 + ≥ 1 | K线无关键价位标注 |
| 14 | 跟踪日志完整 | `grep -c '催化剂兑现追踪\|已兑现\|待兑现' 个股研究-XXX.html` + `grep -c '📋 事件记录' 个股研究-XXX.html` | ≥ 1 + ≥ 1 | 跟踪系统缺失 |
| 15 | 分析日期在nav | `grep -c 'nav-analysis-date' 个股研究-XXX.html` | ≥ 1 | nav无分析日期 |
| 16 | OHLC代码正确 | `grep -c 'ohlc.push(\[d\[1\], d\[4\]' 个股研究-XXX.html` | = 0 | **Bug 6: K线全红无绿** |
| 16b | OHLC代码正确 | `grep -c 'ohlc.push(\[d\[1\], d\[2\]' 个股研究-XXX.html` | ≥ 1 | **Bug 6 验证** |
...
- [ ] 浅色模式切换后：nav logo文字可见、卡片内文字可见、SVG文字可见、KPI数值可见
- [ ] 近期动态无滚动条（目视确认无 `overflow-y` 效果）

**失败处理规则：**
1. 不推导原因、不猜测意图
2. 直接 `grep` 中国长城范例对应结构 → `grep` 自己写的对应位置 → 看差异 → Edit 修正
3. 修正后重跑同一条校验命令，通过才继续
4. 全部 16 条 + 补充检查通过 → 输出最终文件
